...
class FaceEyeEstimatorMediaPipeSync:
    """
    Synchronous per-frame variant of the MediaPipe face/eye estimator.

    Uses RunningMode.VIDEO and detect_for_video() so each call to detect()
    returns results for that specific frame (no async callback, no frame reuse).
    """
    ...
    def __init__(self, input_size: Tuple[int, int]):
        ...
        print(
            f"[infer] FaceLandmarker (sync) ready  model={model_path}  "
            f"closed_threshold={self.closed_threshold}",
            flush=True,
        )
    ...
    def detect(
        ...
        # Eye keypoints (aligned with async variant indices).
        left_idx = [33, 133, 159, 145, 158, 153]
        right_idx = [362, 263, 386, 374, 385, 380]
        left_pts: list[tuple[int, int]] = []
        right_pts: list[tuple[int, int]] = []
        ...
        yaw_deg, pitch_deg, roll_deg = FaceEyeEstimatorMediaPipe._extract_head_pose(result)
