...
class USBCameraSource(CameraSource):
    ...
    def read(self) -> FrameBundle:
        ...
        # If the camera already delivers the desired capture size, avoid an extra resize.
        h, w = frame.shape[:2]
...
def open_camera(headless: bool = False) -> tuple[CameraSource, str]:
    ...
    def _sizes(is_usb: bool) -> tuple[tuple[int, int], tuple[int, int]]:
        # On Pi4 + USB, default to a lower inference resolution for better FPS.
        default_cap_w = 1280
        default_cap_h = 720
        default_inf_w = 640
        default_inf_h = 360
        if not is_usb:
            # PiCamera2 path keeps same defaults unless overridden by env.
            default_inf_w = 1280
            default_inf_h = 720

        cap_w = _env_int("CAPTURE_WIDTH", default_cap_w)
        cap_h = _env_int("CAPTURE_HEIGHT", default_cap_h)
        inf_w = _env_int("INFER_WIDTH", default_inf_w)
        inf_h = _env_int("INFER_HEIGHT", default_inf_h)
