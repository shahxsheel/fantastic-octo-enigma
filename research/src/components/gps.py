...
class GPSReader:
    ...
    def start(self):
        ...
        if self.force_fake:
            ...
            self._set_fake_data()
            print("GPS fake mode enabled from configuration")
        else:
            try:
    ...
    def _set_fake_data(self):
        ...
        with self._lock:
            ...
            self._data['speed_mph'] = 18.0
            self._data['heading'] = 0.0
            self._data['has_fix'] = True

    def _update_fake_data(self):
        """Generate made-up but smooth GPS movement around Apple Park."""
        t = time.time() - self._fake_t0
        angle_deg = (t * 7.5) % 360.0
        angle_rad = math.radians(angle_deg)

        lat = self.APPLE_PARK_LAT + 0.0012 * math.sin(angle_rad)
        lon = self.APPLE_PARK_LON + 0.0012 * math.cos(angle_rad)
        speed_mph = max(0.0, 20.0 + 6.0 * math.sin(t / 4.0))
        heading = (angle_deg + 90.0) % 360.0
        satellites = 8 + int((math.sin(t / 6.0) + 1.0) * 2.0)  # 8..12

        with self._lock:
            self._data['latitude'] = lat
            self._data['longitude'] = lon
            self._data['satellites'] = satellites
            self._data['speed_mph'] = speed_mph
