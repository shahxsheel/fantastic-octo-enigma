...
class GyroReader:
    ...
    def __init__(
        ...
        self.allow_fake_fallback = allow_fake_fallback
        self._use_fake_data = force_fake
    ...
    def start(self, print_from_loop=False):
        ...
        if self.force_fake:
            self._use_fake_data = True
            print("Gyro fake mode enabled from configuration")
        else:
            try:
                import serial
                self._ser = serial.Serial(self.port, self.baud, timeout=0)
                self._use_fake_data = False
            except Exception as e:
                if not self.allow_fake_fallback:
                    raise
                self._use_fake_data = True
                print(f"Gyro unavailable ({e}), using fake gyro data")
    ...
    @property
    def is_fake(self):
        return self._use_fake_data

    def _update_fake_data(self):
        """Generate smooth made-up gyro/acc data when hardware is unavailable."""
        t = time.time() - self._fake_t0
        accx = 0.02 * math.sin(t * 1.7)
        accy = 0.03 * math.cos(t * 1.3)
        accz = 1.0 + 0.01 * math.sin(t * 0.8)
        gyrox = 8.0 * math.sin(t * 1.1)
        gyroy = 6.0 * math.cos(t * 0.9)
        gyroz = 5.0 * math.sin(t * 0.7)
            ...
            if self._use_fake_data:
                ...
                if self._print_from_loop and now - self._last_print >= self.print_interval:
                    if self._measurement_count > 0:
                        print(
                            f"Acceleration magnitude: {self._last_acc_mag:.3f} g | "
                            f"Δg: {self._last_acc_delta:.3f} g | "
                            f"Gyro: {self._last_gyro_mag:.2f} deg/s | "
                            f"M: {self._measurement_count} (fake)"
                        )
                    self._measurement_count = 0
