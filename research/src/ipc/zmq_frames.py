...
class FramePublisher:
    ...
    def send(self, frame_bgr: np.ndarray, frame_id: int, ts_ms: int | None = None) -> None:
        ...
        # IMPORTANT (perf): avoid frame_bgr.tobytes() which forces a full copy per frame.
        # pyzmq can send buffer-protocol objects zero-copy when copy=False.
        if not frame_bgr.flags["C_CONTIGUOUS"]:
            frame_bgr = np.ascontiguousarray(frame_bgr)
        self.sock.send_multipart(
            [json.dumps(header.to_dict()).encode("utf-8"), frame_bgr],
            copy=False,
        )
