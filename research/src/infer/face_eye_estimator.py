...
missing, self.enabled = False and estimate() returns ("UNKNOWN", 1.0, False)
without raising.
"""
...
_RIGHT_EYE = (263, 387, 385, 380, 373, 362)
...
class FaceEyeEstimator:
    ...
    def estimate(
        ...
        """
        Returns (head_direction, avg_ear, is_drowsy).
        ...
        if frame_idx % self.every_n != 0:
            return self._last_direction, self._last_ear, self._last_is_drowsy
        ...
        if not result.face_landmarks:
            ...
            return "UNKNOWN", self._last_ear, self._last_is_drowsy
        ...
        return direction, avg_ear, is_drowsy
    ...
    def _compute_head_direction(self, result, landmarks) -> str:
        ...
        if (
            ...
            try:
                yaw = math.degrees(math.atan2(float(m[1][0]), float(m[0][0])))
                pitch = math.degrees(
                    math.atan2(
                        -float(m[2][0]),
                        math.sqrt(float(m[2][1]) ** 2 + float(m[2][2]) ** 2),
                    )
                )
