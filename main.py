...
def main() -> int:
    ...
    p.add_argument(
        "--single-usb",
        action="store_true",
        help="Run single-process USB pipeline (YOLO + face/eye every frame).",
    )
    args = p.parse_args()

    if args.single_usb:
        # Single-process USB runner lives in the same Python environment.
        from src.run_single_usb import main as run_single_usb

        return run_single_usb()
