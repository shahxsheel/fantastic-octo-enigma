-- ================================================================
-- Hobby-Mode Supabase Setup (idempotent)
-- Removes Shazam / face-recognition / live-frame streaming surfaces
-- and keeps public-mode contracts for GPS, Gyro, YOLO and buzzer.
-- ================================================================

create extension if not exists pgcrypto;

-- ----------------------------------------------------------------
-- Cleanup: deprecated feature artifacts
-- ----------------------------------------------------------------
...
-- Remove old storage policies/buckets for removed features.
...
alter table if exists public.user_profiles add column if not exists created_at timestamptz not null default now();
alter table if exists public.user_profiles add column if not exists updated_at timestamptz not null default now();
alter table if exists public.user_profiles add column if not exists display_name text;
alter table if exists public.user_profiles add column if not exists avatar_path text;
alter table if exists public.user_profiles add column if not exists notification_preferences jsonb not null default
  '{"collision": true, "driver_drowsiness": true, "speed_limit": true, "drunk_driving": true, "fsd": true}'::jsonb;
alter table if exists public.user_profiles add column if not exists notifications_enabled boolean not null default true;
alter table if exists public.user_profiles add column if not exists push_token text;
...
-- Kept for backward compatibility only (deprecated in hobby mode).
...
alter table if exists public.vehicle_access add column if not exists created_at timestamptz not null default now();
alter table if exists public.vehicle_access add column if not exists access_level text not null default 'viewer';
...
create index if not exists idx_vehicle_realtime_updated_at on public.vehicle_realtime(updated_at desc);
create index if not exists idx_vehicle_realtime_is_speeding on public.vehicle_realtime(is_speeding);
...
create unique index if not exists idx_vehicle_access_user_vehicle on public.vehicle_access(user_id, vehicle_id)
where user_id is not null;

-- ----------------------------------------------------------------
-- Storage bucket (keep only user avatars)
-- ----------------------------------------------------------------
...
create policy "hobby_vehicles_select_public"
on public.vehicles
for select
...
create policy "hobby_vehicles_insert_public"
on public.vehicles
for insert
to anon, authenticated
with check (true);
...
create policy "hobby_vehicles_update_public"
on public.vehicles
for update
to anon, authenticated
using (true)
...
create policy "hobby_vehicles_delete_public"
on public.vehicles
for delete
to anon, authenticated
using (true);
...
create policy "hobby_vehicle_realtime_select_public"
...
create policy "hobby_vehicle_realtime_insert_public"
on public.vehicle_realtime
for insert
to anon, authenticated
with check (true);
...
create policy "hobby_vehicle_realtime_update_public"
on public.vehicle_realtime
for update
to anon, authenticated
using (true)
...
create policy "hobby_vehicle_trips_select_public"
...
create policy "hobby_vehicle_trips_insert_public"
on public.vehicle_trips
for insert
to anon, authenticated
with check (true);
...
create policy "hobby_vehicle_trips_update_public"
on public.vehicle_trips
for update
to anon, authenticated
using (true)
...
-- user_profiles (auth-only self-service)
drop policy if exists "hobby_user_profiles_select_own" on public.user_profiles;
create policy "hobby_user_profiles_select_own"
...
create policy "hobby_user_profiles_insert_own"
on public.user_profiles
for insert
to authenticated
...
create policy "hobby_user_profiles_update_own"
on public.user_profiles
for update
to authenticated
using (auth.uid() = user_id)
...
create policy "hobby_user_profiles_delete_own"
on public.user_profiles
for delete
to authenticated
using (auth.uid() = user_id);

-- vehicle_access (deprecated)
drop policy if exists "hobby_vehicle_access_service_role_all" on public.vehicle_access;
create policy "hobby_vehicle_access_service_role_all"
on public.vehicle_access
for all
to service_role
using (true)
with check (true);

drop policy if exists "hobby_vehicle_access_select_self" on public.vehicle_access;
create policy "hobby_vehicle_access_select_self"
on public.vehicle_access
for select
to authenticated
using (auth.uid() = user_id);
...
drop policy if exists "hobby_user_avatars_select_public" on storage.objects;
create policy "hobby_user_avatars_select_public"
on storage.objects
for select
to anon, authenticated
...
drop policy if exists "hobby_user_avatars_insert_own" on storage.objects;
create policy "hobby_user_avatars_insert_own"
on storage.objects
for insert
to authenticated
with check (
...
drop policy if exists "hobby_user_avatars_update_own" on storage.objects;
create policy "hobby_user_avatars_update_own"
on storage.objects
for update
to authenticated
using (
...
drop policy if exists "hobby_user_avatars_delete_own" on storage.objects;
create policy "hobby_user_avatars_delete_own"
on storage.objects
for delete
to authenticated
using (
...
-- ----------------------------------------------------------------
-- Buzzer RPCs (no vehicle_access dependency)
-- ----------------------------------------------------------------
...
begin
  ...
  insert into public.vehicle_realtime (
    vehicle_id,
    buzzer_active,
    buzzer_type,
    buzzer_updated_at,
...
-- ----------------------------------------------------------------
-- Realtime publication guards (idempotent)
-- ----------------------------------------------------------------
